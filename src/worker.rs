...
async fn spawn_worker(database: &Database) -> (Sender<(SubmissionId, TestId, Arc<PathBuf>)>, Arc<AtomicI32>) {
...
impl WorkerManager {
    pub async fn new(worker_count: usize, database: &Database) -> WorkerManager {
        ...
        WorkerManager { workers: Arc::new(workers) }
    }

    async fn execute_test(&self, submission_id: SubmissionId, test_id: TestId, executable: Arc<PathBuf>) {
        ...
        sender.send((submission_id, test_id, executable)).await.unwrap();
    }
